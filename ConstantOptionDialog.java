...
public class ConstantOptionDialog extends FilterDialog {
	private final ConstantOptionDialog thisDialog = this;
	private double offset;
	private boolean retVal = false;

	public ConstantOptionDialog(final JFrame thisFrame, 
							   String title, 
							   boolean modal, 
							   final ECGViewHandler handler,
							   final int index) {
		
		super(thisFrame, title, modal, handler, index, 6);
		...
		controls.add(new JLabel("Offset"), labels);
		final JLabel leftNum = new JLabel("0");
		controls.add(leftNum, values);
		final JSlider leftSlide = new JSlider(-100000, 100000, 000);
		...
		accept.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				offset = ((double)(int)leftSlide.getValue())/100.0;
